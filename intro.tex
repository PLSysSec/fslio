...
Unfortunately, introducing flow-sensitive references to purely dynamic IFC
...
security levels, which are respectively |low| and |high|y. In contrast, variable |t|
...
To close this covert channel, there exist mainly three different proposals:
...
changes ---a policy known as \emph{no-sensitive
  upgrades}~\cite{Austin:Flanagan:PLAS10}---, and deny branches on certain
...
systems Asbestos~\cite{Efstathopoulos:2005}, HiStar~\cite{zeldovich:histar},
and Flume~\cite{krohn:flume}. In a nutshell, those systems are composed of
...
(except for~\cite{Krohn,seL4IF}). Bear in mind that an operating system is not
...
systems~\cite{sabelfeld:language-based-iflow} or as tainting in secure OS.  In
...
increase permissiveness when interacting with external databases~\cite{giffin:hails}.
...
discipline~\cite{Austin:Flanagan:PLAS10}). Inspired by~\cite{Hedin13}, we add
...
channel should not be neglected~\cite{stefan:addressing-covert}, our upgrade
...
Although developed for LIO, our approaches for the sequential
and concurrent settings are applied to other language-based floating-label
systems (e.g., \cite{10.1109/SP.2013.10, Efstathopoulos:2005, zeldovich:histar,
krohn:flume}).
