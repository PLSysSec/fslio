...
Information-flow control (IFC) is a promising approach to preserve
...
sensitive, or secret, data into public channels.  When no leaks are desired, the
...
monitor~\cite{Russo:2010,stefan:lio,Breeze}; forbid certain label
changes, a policy known as \emph{no-sensitive
  upgrades} \Red{(first proposed by Zdancewic~\cite{Zdancewic02programminglanguages} and named by Austin and Flanagan~\cite{Austin:Flanagan:PLAS09})}; and deny branches on certain
...
flow-insensitive concepts---the key idea being the ability to have nested labeled
values.  
...
sequential and concurrent floating-label systems (e.g.,
\cite{Breeze, Efstathopoulos:2005, zeldovich:histar, krohn:flume}).
