...
confidentiality of the value's label. The calculus then forbids to change a
label of a reference if \emph{the label on the label is below the
  floating-label} (i.e. following the {\nsu}
discipline~\citep{Austin:Flanagan:PLAS10}). Inspired by~\citep{Hedin13}, we add
an instruction capable to \emph{upgrade} labels under certain safe conditions in
...
flow-insensitive concepts---the key idea being to considered nested labeled
...
To reduce the programmer's burden to
...
flow-sensitive references, while forbidden the access to undeclared ones.
...
\item A uniform mechanisms to treat flow-insensitive and flow-sensitive
