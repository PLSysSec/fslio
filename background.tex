...
>>= \x.return (x+1)|, which describes a computation that adds one to the value
...
In fact, labels are actually protected by the current label. Since
...
%
We use subscript |s| to respectively distinguish flow-insensitive (\textrm{FI}) and
flow-sensitive (\textrm{FS}) productions (the former are explained in this section, and the
