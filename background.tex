...
We describe a calculus which gives precise semantics to |LIO| and the rôle of
...
implicit flows. A branch condition, which are simply values of type |Bool|, are
already labeled as |lcurr|. Consequently, all the subsequent writes cannot leak
...
of code cannot, for example, and independently of any secret, write the public
constant |42| to a public channel labeled |low| after observing secret
data---once secret data is read, the current label is raised to |high| thereby
...
depend on |t|. 
%
Moreover, this rules highlights the fact that the current label always
...
allows to  \emph{temporarily} raise the current label during the execution of 
a given computation. 
...
would allow for trivial leaks, and therefore |toLabeled| labels |t'| with |l| 
(|conf c'' (E[label l t']|).
%
...
reference amounts to ensure reference label to be as restrictive as the current
...
flow-insensitive and flow-sensitive productions (The former explain in this
...
%
Subsequently, the function returns a value of shape |LIORefTCB I l a| (with type
