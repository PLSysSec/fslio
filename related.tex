...
more permissive than the former (i.e. it rejects less suspicious programs). The
{\nsu} discipline stops the execution on any attempt to change the label of a
...
The flow-sensitive extension for LIO is capable to encode a generalize version
of {\nsu} by using an extra level of indirection.  To illustrate that, we
consider all the references as a three-level nested labeled values---perfectly
...
as effectively having the first and second level of the structure collapsed. For
...
for an arbitrary label |l''|.  The new created reference is of the form
|LabeledTCB lcurr (LabeledTCB lcurr lv)|.  As in {\nsu}, writing a (labeled)
value into r, |writeRef S r lv'|, demands that the current label flows into
|lcurr|, i.e., the label on the label at the time of creating |r|. This
restriction rules out situations where, if |r| was creating in a public
environment (|low|), it is not possible to modify it in a secret context
(|high|). These are exactly the forbidden label changes in {\nsu}. Additionally,
|writeRef S r lv'| allows to change the initially stored labeled value |lv| by
