...
%
\Red{In the embedding of |writeRef S|, we use |toLabeled| to make any
  changes to the current label (possibly caused by reading the
  reference) local to this operation. Inside |toLabeled|, the code
  fetches the labeled value from the reference (|readRef I|), and then
  performs the security check as in the rule in
  Figure~\ref{fig:sos:fs}. If this succeeds, a new labeled value that
  wraps |t| is made (|label|), and then stored in the reference
  (|writeRef I|).}
%
...
\Red{
  ...
  Our flow-sensitive calculus is as permissive as flow-insensitive
  LIO. Any flow-insensitive LIO program can be trivially converted to
  a flow-sensitive LIO program (without auto-upgrades) by using
  flow-sensitive references instead of flow-insensitive ones.  Since
  these references would never be upgraded, they will behave just like
  their flow-insensitive counterparts.

  This means that all existing LIO programs can be run in our
  flow-sensitive monitor. As an example of such a program, we
  highlight Hails~\cite{giffin:hails}, a web framework using LIO, on
  top of which a number of applications have been built (e.g.,
  GitStar\footnote{\url{www.gitstar.com}}, a code-hosting web platform, LearnByHacking\footnote{\url{www.learnbyhacking.org}}, a
  blog/tutorial platform similar to School of Haskell, and
  LambdaChair~\cite{stefan:2012:arxiv-flexible}, an EasyChair-like conference review system).
}
