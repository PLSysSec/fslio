...
%
Additionally, it almost always over-approximates the sensitivity of the
...
(e.g. \cite{Russo:2010,Austin:Flanagan:PLAS09,Austin:Flanagan:PLAS10}),
this approach is not safe; consider the code in
Figure~\ref{fig:fs-attack}. \Red{The function |when| acts like an |if|
  statement without the |else| branch, and |($)| is a lightweight
  notation for function application (|f $ x| is the same as |f
  (x)|) used to reduce the number of parentheses in the expression.}
...
%\noindent
%
In this attack, |leakRef| can be used to leak the value stored in the secret reference
|href|, while keeping the current label |low|, by leveraging a new
covert channel: the label of references.
%
Concretely, |leakRef| creates a public reference |tmp| and, within the
...
%
\Red{In certain situations, this condition ensures that the enforcement is sufficiently
permissive. For example, consider a flow-sensitive reference with a |high| label
and with a |low| label on the label. If we checked the conditions
|lcurr canFlowTo l| and |lcurr canFlowTo l'| independently, then it
would not be possible to write to this reference when
|lcurr| is |high|.}
%
...
%
If the semantics got stuck in \ruleref{writeRef-fs}, the current label would
...
%
It ensures that the current computation can modify the label by
...
triggered, returning a value; we specify this big-step rule in terms of
...
%
We lastly remark the elimination of 
the aforementioned attack by simply wrapping |toLabeled| with |withRefs|,
thereby preventing the (untrusted) code within the |toLabeled| block from
upgrading arbitrary references. 
