...
\Red{
- fs good; can address log problem
- upgrade is painful, want auto upgrades
- need withRefs to ensure we're not upgrading the whole store
  natural place to use withRefs is in tolabeled
- talk about no sensitive upgrades
}
...
{|c = (lcurr, mI, mS)|\\
 ...
 |ti = upgradeR (LIORefTCB S a_i) lcurr, i = 1, ..., n|
}
{|conf c (E[upgradeM]) --> conf c (E[t1 >> ... >> t_n])|}
\and
...
{
|conf c (E[unlabel (LabeledTCB l t)]) --> conf c' (E[t'])|
}
\end{mathpar}
