...
concurrrency; to our knowledge, this has not been considered by other works.
%
Specifically, we consider a subset of the concurrent version of
LIO~\tocite{ifcp}, that extends our sequential \liofs{} calculus (parametric in
the auto-upgrade policy) with threads and a new terminal, |forkLIO|, for
dynamically creating new threads.
%
However, to guarantee termination-sensitive non-interfernce, |toLabeled| must
be removed from the surface syntax of the underlying sequential calculus, as
in~\tocite{ifcp}.\footnote{
As discussed in~\tocite{icfp}, |forkLIO| with synchronization primitives can be
used to provide functionality equivalent to that of |toLabeled|.
}
%
Intuitively, our concurrent calculus is simply a scheduler; taking a step in
the concurrent calculus amounts to taking a step in a sequential thread and
context switching to a different thread.
%
...
%
(To this end we extend the reduction relation of the underlying sequential calculus with events; since |fork| is the only event, we do not disucss this further.)
%
